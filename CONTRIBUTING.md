...
By contributing to AppVeyor CLI, you assert that:

* The contribution is your own original work.
* You have the right to assign the copyright for the work (it is not owned by your employer, or
  you have been given copyright assignment in writing).
* You [license](https://github.com/jrgcubano/appveyor-cli/blob/main/LICENSE) the contribution under the terms applied to the rest of the AppVeyor CLI project.
* You agree to follow the [code of conduct](https://github.com/jrgcubano/appveyor-cli/blob/main/CODE_OF_CONDUCT.md).
...
This project targets .NET 10 with C# 14. Please use modern language features where appropriate
(records, primary constructors, pattern matching, etc.).

### Dependencies

Keep dependencies minimal. The core CLI should only depend on:

* Spectre.Console / Spectre.Console.Cli (CLI framework and rich output)
* Microsoft.Extensions.DependencyInjection (DI container)
* Microsoft.Extensions.Http (HttpClient factory)
* System.Text.Json (serialization, via source generators)

Any new dependency should be discussed in an issue before being added.
...
Or using the Cake build script:
...
* Through GitHub, or through [GitHub Issues](https://github.com/jrgcubano/appveyor-cli/issues),
  you talk about a feature you would like to see (or a bug), and why it should be in AppVeyor CLI.
* Once you get a nod from a maintainer, you can start on the feature.
* Alternatively, if a feature is on the issues list with the
  [good first issue](https://github.com/jrgcubano/appveyor-cli/labels/good%20first%20issue) label,
...
* You create, or update, a fork of `jrgcubano/appveyor-cli` under your GitHub account.
...
* Once you feel it is ready, submit the pull request to the `jrgcubano/appveyor-cli` repository against the `main` branch
...
* One of the maintainers will evaluate it within a reasonable time period (which is to say usually
  within 1-3 weeks). Some things get evaluated faster or fast tracked. We are human and we have
  active lives outside of open source so don't fret if you haven't seen any activity on your pull
  request within a month or two. We don't have a Service Level Agreement (SLA) for pull requests.
...
* `git fetch upstream` (upstream remote would be `jrgcubano/appveyor-cli`)
* `git checkout main`
* `git rebase upstream/main`
* `git checkout your-branch`
* `git rebase main`
* Fix any merge conflicts
* `git push origin your-branch` (origin would be your GitHub repo or `your-github-username/appveyor-cli` in this case).
...
This contribution guide was adapted from the [Spectre.Console project](https://github.com/spectreconsole/spectre.console)
which in turn was taken from the [Chocolatey project](https://chocolatey.org/) with permission.
